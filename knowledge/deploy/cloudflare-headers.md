...
`public/_headers` → Astro `dist/` → Cloudflare Pages. Sets `Cache-Control` plus origin-isolation headers for WebMCP. HTML cache stays Pages default (`max-age=0`). `astro preview` does not apply these rules; verify on a real deploy. Project: `aaryanporwal-com`.

# Policies

| Path | Headers | Notes |
|------|---------|-------|
| `/*` | `Origin-Agent-Cluster: ?1`, `Permissions-Policy: tools=(self)` | WebMCP requires origin isolation; `?0` disables `document.modelContext` |
| `/_astro/*` | `Cache-Control` 1y + `immutable` | Fingerprinted; new content → new URL |
...
4. **Never long-cache HTML:** The `/*` rule is for origin isolation only. Do not add long `max-age`/`immutable` there.
