const DEFAULT_USER_AGENT =
  "fix-dead-links/0.1.0 (+https://github.com/aaryanporwal/websoot-v1)";
...
export async function isDeadLink(
  url: string,
...
export function snapshotUrlFromWaybackResponse(data: unknown): string | null {
  const snapshot = (
    data as {
      archived_snapshots?: {
        closest?: {
...
export async function getArchiveUrl(
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      return snapshotUrlFromWaybackResponse(await response.json());
    } catch {
