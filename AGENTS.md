...
- Git hooks live in `.githooks/` (`core.hooksPath`). `bun install` runs `prepare` (`scripts/install-git-hooks.sh`) to point git at them and copy them into `.git/hooks` so Cursor Cloud's dispatcher still runs them.
...
- If you forget, `pre-commit` rewrites the identity and rejects the commit; retry and it will be authored by Aaryan. `commit-msg` also strips `Co-authored-by` trailers. Cursor Cloud may still append a self-trailer *after* our hook; authorship is what GitHub squash uses.
